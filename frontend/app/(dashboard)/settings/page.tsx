...
function ToggleRow({ label, sub, defaultOn = false }: { label: string; sub: string; defaultOn?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between py-4 border-b border-cyan-400/20 last:border-0">
      <div>
        <p className="text-xs font-mono uppercase tracking-wider text-cyan-400">{label}</p>
        <p className="text-xs text-cyan-400/50 mt-0.5">{sub}</p>
      </div>
...
function SliderRow({ label, sub, min, max, defaultVal, unit }: { label: string; sub: string; min: number; max: number; defaultVal: number; unit: string }) {
  ...
  return (
    <div className="py-4 border-b border-cyan-400/20 last:border-0">
...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Tab Selector */}
      <div className="scifi-card p-1.5 flex gap-1 flex-wrap">
        {tabs.map(tab => {
          const Icon = tab.icon
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-none text-xs font-mono uppercase tracking-wider transition-all flex-1 justify-center sm:flex-initial sm:justify-start ${
                activeTab === tab.id
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {activeTab === 'account' && (
            <div className="space-y-4">
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-5">Profile Information</h3>
                <div className="flex items-center gap-5 mb-6 pb-6 border-b border-cyan-400/20">
                  <div className="w-16 h-16 rounded-none bg-gradient-to-br from-cyan-400 to-purple-600 flex items-center justify-center text-2xl font-black text-cyan-400 flex-shrink-0">
                    {name.charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <p className="font-semibold text-cyan-400">{name}</p>
                    <p className="text-sm text-cyan-400/50">{email}</p>
                    <button className="text-xs text-cyan-400 hover:text-cyan-300 mt-1">Change avatar →</button>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="scifi-label text-cyan-400/70 uppercase tracking-wider">Full Name</label>
                    <input
                      id="settings-name"
                  ...
                  <div>
                    <label className="scifi-label text-cyan-400/70 uppercase tracking-wider">Email Address</label>
                    <input
                      id="settings-email"
                      type="email"
                      className="input-field mt-2"
              ...
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-5">Change Password</h3>
                <div className="space-y-4">
                  <div>
                    <label className="scifi-label text-cyan-400/70 uppercase tracking-wider">Current Password</label>
                    <input id="settings-current-pwd" type="password" placeholder="••••••••" className="input-field mt-2" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="scifi-label text-cyan-400/70 uppercase tracking-wider">New Password</label>
                      <input id="settings-new-pwd" type="password" placeholder="••••••••" className="input-field mt-2" />
                    </div>
                    <div>
                      <label className="scifi-label text-cyan-400/70 uppercase tracking-wider">Confirm New</label>
              ...
              <div className="scifi-card p-5 border border-red-500/15">
                <h3 className="text-sm font-semibold text-red-400 mb-2">Danger Zone</h3>
                <p className="text-xs text-cyan-400/50 mb-3">Permanently delete your account and all associated data.</p>
                <button className="btn-secondary px-4 py-2 text-sm rounded-none text-red-400 border-red-500/20 hover:bg-red-500/10">
                  Delete Account
                </button>
              </div>
            </div>
          ...
          {activeTab === 'appearance' && (
            <div className="space-y-4">
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-5">Theme</h3>
                <div className="flex gap-3">
                  {[
                    ...
                    <button
                      ...
                      className={`flex-1 p-4 rounded-none border transition-all text-left ${
                        darkMode === mode.id
                          ? 'bg-cyan-500/10 border-cyan-500/30 text-cyan-400'
                          : 'scifi-card border-white/8 text-cyan-400/50 hover:text-cyan-400/90'
                      }`}
                    >
              ...
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-5">Accent Color</h3>
                <div className="flex gap-3 flex-wrap">
                  {accents.map(a => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-4 py-2.5 rounded-none border text-sm transition-all ${
                        accent === a.id ? 'border-white/30 bg-white/8' : 'border-white/8 text-cyan-400/50 hover:text-cyan-400/90'
                      }`}
                    >
                      <div className="w-4 h-4 rounded-none" style={{ background: a.color }} />
                      {a.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-1">Chart Animation Speed</h3>
          ...
          {activeTab === 'notifications' && (
            <div className="scifi-card p-6">
              <h3 className="text-sm font-semibold text-cyan-400/90 mb-2">Alert Preferences</h3>
              <p className="text-xs text-cyan-400/50 mb-5">Configure when and how EdgeVisionNet notifies you.</p>
              <div>
              ...
              <div className="mt-5 pt-5 border-t border-cyan-400/20">
                <h4 className="scifi-label text-cyan-400/70 uppercase tracking-wider mb-3">Alert Thresholds</h4>
          ...
          {activeTab === 'devices' && (
            <div className="space-y-4">
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-5">Device Preferences</h3>
                <div className="space-y-4">
                  <div>
                    <label className="scifi-label text-cyan-400/70 uppercase tracking-wider">Default Device ID</label>
                    <input
              ...
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-2">Telemetry Settings</h3>
          ...
          {activeTab === 'about' && (
            <div className="space-y-4">
              <div className="scifi-card p-8 text-center border border-cyan-500/15">
                <div className="w-16 h-16 rounded-none bg-gradient-to-br from-cyan-400 to-purple-600 flex items-center justify-center mx-auto mb-4 shadow-glow-cyan">
                  <Cpu size={28} className="text-cyan-400" />
                </div>
                <h2 className="text-2xl font-black gradient-text mb-1">EdgeVisionNet</h2>
                <p className="text-cyan-400/50 text-sm mb-3">Real-Time AI Energy Intelligence Platform</p>
                <span className="badge-pro">v1.0.0 PRO</span>
              </div>

              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-4">Technology Stack</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={name} className="scifi-card p-3">
                      <p className="scifi-label text-cyan-400">{name}</p>
                      <p className="text-[10px] text-cyan-400/50 mt-0.5">{desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="scifi-card p-6">
                <h3 className="text-sm font-semibold text-cyan-400/90 mb-4">Links & Resources</h3>
                  ...
                  {[
                    ...
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center gap-3 p-3 rounded-none hover:bg-white/4 transition-colors group"
                      >
                        <div className="w-9 h-9 rounded-none bg-cyan-400/5 flex items-center justify-center">
                          <Icon size={16} className="text-cyan-400/70 group-hover:text-cyan-400 transition-colors" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-mono uppercase tracking-wider text-cyan-400/90">{item.label}</p>
                          <p className="text-xs text-slate-600 truncate">{item.sub}</p>
                        </div>
                        <ChevronRight size={14} className="text-slate-600 group-hover:text-cyan-400/70 transition-colors" />
                      </a>
                    )
                  })}
                </div>
