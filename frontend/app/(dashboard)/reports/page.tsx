...
import {
  Download, Leaf, Zap, Brain, BarChart3,
...
const summaryCards = [
  { label: 'Weekly Energy Saved', value: '14.7 Wh',     sub: 'vs cloud baseline (12W)', icon: Leaf,    color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', trend: 'up',     trendVal: '+12.3%' },
  { label: 'Monthly AI Usage',    value: '2,847',        sub: 'Predictions this month',  icon: Brain,   color: 'text-purple-400',  bg: 'bg-purple-500/10',  border: 'border-purple-500/20',  trend: 'up',     trendVal: '+24.1%' },
  { label: 'Top Model',           value: 'EdgeVisionNet',sub: '74.2% avg accuracy',      icon: BarChart3,color:'text-cyan-400',   bg: 'bg-cyan-500/10',    border: 'border-cyan-500/20',    trend: 'neutral',trendVal: 'Stable' },
  { label: 'CO₂ Reduction',       value: '0.038 kg',     sub: 'Saved vs cloud this month',icon: Zap,   color: 'text-amber-400',   bg: 'bg-amber-500/10',   border: 'border-amber-500/20',   trend: 'up',     trendVal: '+8.5%'  },
]
...
const MODEL_COLORS: Record<string, string> = {
  'EdgeVisionNet': '#00d4ff',
  'MobileNetV2':   '#8b5cf6',
  'EfficientNet-B0': '#10b981',
  'ShuffleNet':    '#f59e0b',
}
        ...
        <div>
          <div className="flex items-center gap-2.5 mb-1">
            <span className="badge-online">
              <span className="pulse-dot" /> Auto-generated
            </span>
            <span className="text-xs text-slate-500 font-mono">
              {new Date().toLocaleDateString('en', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            </span>
          </div>
          <h1 className="page-title">Energy & AI Reports</h1>
          <p className="page-subtitle">Performance summaries and downloadable analytics</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {summaryCards.map((card, i) => {
          ...
          return (
            ...
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${card.bg} opacity-20 pointer-events-none`} />
              <div className="relative">
                ...
                <p className="text-[11px] text-slate-600 mb-2">{card.sub}</p>
                <div className={`flex items-center gap-1 text-xs ${
                  card.trend === 'up' ? 'text-emerald-400' : card.trend === 'down' ? 'text-red-400' : 'text-slate-500'
                }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-200">7-Day Energy Overview (Wh)</h3>
            <div className="flex items-center gap-3 text-[10px] text-slate-500">
              <span className="flex items-center gap-1.5"><div className="w-3 h-0.5 rounded bg-emerald-400" /> Saved</span>
              <span className="flex items-center gap-1.5"><div className="w-3 h-0.5 rounded bg-cyan-400" /> Used</span>
            </div>
          </div>
        ...
        <div className="glass-card p-5">
          <h3 className="text-sm font-semibold text-slate-200 mb-4">Daily Predictions (Last 30 Days)</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={monthlyPredictions} margin={{ top: 4, right: 4, left: -20, bottom: 0 }} barSize={6}>
              ...
              <Bar dataKey="count" name="Predictions" radius={[4, 4, 0, 0]}>
                {monthlyPredictions.map((_, i) => (
                  <Cell key={i} fill={i % 2 === 0 ? 'rgba(0,212,255,0.7)' : 'rgba(139,92,246,0.7)'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      ...
                      style={{ color: MODEL_COLORS[p.model] ?? '#00d4ff' }}
                    >{p.model}</span>
                  </td>
                  <td className="px-4 py-3 text-sm font-semibold text-white">{p.class}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-14 bg-white/5 rounded-full h-1.5 overflow-hidden">
                        <motion.div
                          className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-purple-500"
                          initial={{ width: 0 }}
                          animate={{ width: `${p.conf * 100}%` }}
