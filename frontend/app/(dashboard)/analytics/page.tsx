...
import { telemetryAPI, reportsAPI } from '@/lib/api'
import type { TelemetryHistory } from '@/lib/types'
import LiveChart from '@/components/LiveChart'
import { BarChart3, Clock, Download, RefreshCw, TrendingUp, Activity } from 'lucide-react'
...
export default function AnalyticsPage() {
  ...
  const exportCSV = () => {
    ...
    const rows   = data.map(r => `${r.timestamp},${r.cpu},${r.ram},${r.battery},${r.temp},${r.power_w},${r.charging}`).join('\n')
  ...
  const summaryCards = [
    { label: 'Avg CPU',      value: `${stats.avgCpu.toFixed(1)}%`,    color: 'text-cyan-400',    bg: 'bg-cyan-500/10',    border: 'border-cyan-500/20',    icon: <Activity size={18} /> },
    { label: 'Avg RAM',      value: `${stats.avgRam.toFixed(1)}%`,    color: 'text-purple-400',  bg: 'bg-purple-500/10',  border: 'border-purple-500/20',  icon: <BarChart3 size={18} /> },
    { label: 'Avg Temp',     value: `${stats.avgTemp.toFixed(1)}°C`,  color: 'text-amber-400',   bg: 'bg-amber-500/10',   border: 'border-amber-500/20',   icon: <TrendingUp size={18} /> },
    { label: 'Avg Power',    value: `${stats.avgPower.toFixed(2)}W`,   color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', icon: <Clock size={18} /> },
    { label: 'Data Points',  value: stats.count.toString(),            color: 'text-cyan-400',    bg: 'bg-cyan-500/10',    border: 'border-cyan-500/20',    icon: <BarChart3 size={18} /> },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="page-subtitle">Telemetry trends over configurable time windows</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <motion.button
            ...
            className="btn-primary px-3.5 py-2 text-sm flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {summaryCards.map((card, i) => (
          ...
          >
            <div className={`${card.bg} w-8 h-8 rounded-lg flex items-center justify-center ${card.color} mb-3`}>
              {card.icon}
            </div>
      ...
      {loading ? (
        ...
        <div className="glass-card p-16 text-center">
          ...
          <p className="text-slate-400 font-medium">No telemetry data for this time range</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <LiveChart id="hist-ram"      title="RAM Usage (%)"           data={data} series={[{ key: 'ram',     label: 'RAM',    color: '#8b5cf6' }]} unit="%" />
          ...
          <LiveChart id="hist-combined" title="CPU + RAM Overlay (%)" data={data} type="line" series={[{ key: 'cpu', label: 'CPU', color: '#00d4ff' }, { key: 'ram', label: 'RAM', color: '#8b5cf6', dashed: true }]} unit="%" />
        </div>
      )}
    </div>
  )
